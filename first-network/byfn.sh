...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.sfiot.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.sfiot.com
    docker cp -a orderer.sfiot.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.sfiot.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.sfiot.com

    for PEER in peer0.org1.sfiot.com peer1.org1.sfiot.com peer0.org2.sfiot.com peer1.org2.sfiot.com; do
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_ORG4 down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx channel-artifacts/*.txt crypto-config/ ./org4-artifacts/crypto-config/ channel-artifacts/org4.json
...
# (``peer0.org1.sfiot.com`` & ``peer0.org2.sfiot.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/Or32MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org3MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
