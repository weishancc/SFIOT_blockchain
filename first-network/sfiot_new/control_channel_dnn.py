...
#==variable=====================================================================================================
CONTAINER_NAME_DNN = "container_D_0"     # BC container's name
...
class subthread_dnn():
    def __init__(self):
        ...
        self.socket_temp = []
#--------------------------------------------------------------------------------------------------------------
    def dnn_addThread_for_listen(self, port):
...
if __name__ == "__main__":
    control_channel = subthread_dnn() #DNN main thread
    DNN_receive_from_iot_thread = th.Thread(target = BC_receiver_run, args=(0, 10000))    #first data thread witch Receiving data from iot always listen on 10000 port.
    DNN_receive_from_iot_thread.start()
    print("The port witch receiving data from iot is listening!, port: 10000")

    control_channel.thread.append(th.Thread(target = BC_receiver_run, args=(0, 6000)))     #first data thread always listen on 6000 port.  
    control_channel.port.append(6001)           #Storing the thread witch receiving data from DNN to port queue.
    # control_channel.thread[-1].start()        #Data thread start.
    # print(control_channel.thread)
    control_channel.run(CONTAINER_PORT_DNN)    #Control thread start.
