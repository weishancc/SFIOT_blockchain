...
#from socket.client_for_outside import * 
...
class LoRaRcvCont(LoRa):
    ...
    def on_rx_done(self):
        ...
        pl = ''.join([chr(c) for c in payload])
        print("payload size: {} bytes".format(sys.getsizeof(pl)))
        data = QuBuf(pl)
        print(data)
        ...
        try:
            ...
            try:
                # python3 unicode
                #print("\nReceive: {}".format( _data.encode('latin-1').decode('unicode_escape'))) #raw code --annotation by yiyu
                #data = "{}".format( _data.encode('latin-1').decode('unicode_escape'))
                data = str( _data.encode('latin-1').decode('unicode_escape'))
                print("\nReceive:{}".format(data.strip('\n')))
                socket_connect(data)
            except:
                # python2
    ...
    '''def prossesing_data(self,prodata):
        prodata = listen_network_outside()
        return prodata
    '''
...
try:
    lora.start()
    #on_rx_done() #import client of socket connect --yiyu
except KeyboardInterrupt:
    sys.stdout.flush()
